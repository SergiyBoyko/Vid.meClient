...
public class FeedFragment extends BaseFragment implements FeedContentView {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        token = getApp().getUserProfile().getAuth().getToken();
    ...
    public void networkConnected() {
        if (adapter.getItemCount() <= offset)
            presenter.getFeedContent(Constants.VIDEO_COUNT_LIMIT, offset, token);
    ...
    private void loadFragmentSources() {
        ...
        presenter.getFeedContent(Constants.VIDEO_COUNT_LIMIT, 0, token);
    }
